...
//Add Invoice Field
function addInputField(divName, t_count = 2) {
    if(count<t_count){
        count = t_count;
    }
    if (count == limits) {
...
var p_count = 1;
function addProductRow(divName,b_count = 1) {
    r_count = b_count;
    if (p_count == limits) {
        ...
        newdiv.innerHTML = 
            '<td><input type="text" name="product_name_' + r_count + '[]" onkeyup="invoice_productList('+ r_count +');" class="form-control productSelection" placeholder="'+display('product_name')+'" required="" id="product_name_' + r_count + '" /><input type="hidden" class="autocomplete_hidden_value product_id_' + p_count + '" name="product_id_' + r_count + '[]"/><input type="hidden" class="sl" value="' + p_count + '"/><input type="hidden" class="baseUrl" value="'+base_url+'" /></td>'+
            '<td><input type="radio" name="is_default_'+ r_count +'" class="default_box is_default_'+ r_count +'" value="1"></td>'+
...
var f_count = 1
function addFirstProductRow(divName,a_count = 1){
    if(f_count<a_count){
        f_count = a_count;
    }
    if (f_count == limits) {
