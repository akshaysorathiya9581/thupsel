...
if (!function_exists('settingsKeys')) {
    function settingsKeys()
    {
        return $settingsKeys = [
            ...
            'FROM_EMAIL' => "",
            'FROM_NAME' => "",
            "estimation_number_prefix" => "#EST-000",
            "workorder_number_prefix" => "#WO-000",
...
if (!function_exists('assignSubscription')) {
    function assignSubscription($id)
    {
        ...
        if ($subscription) {
            ...
            $users = User::where('parent_id', '=', parentId())->whereNotIn('type', ['client'])->get();
...
if (!function_exists('estimationPrefix')) {
    function estimationPrefix()
    {
        $settings = settings();
        return $settings["estimation_number_prefix"];
    }
}
...
if (!function_exists('workOrderPrefix')) {
    function workOrderPrefix()
    {
        $settings = settings();
        return $settings["workorder_number_prefix"];
    }
}
